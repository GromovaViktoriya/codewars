...
function balance(book) {
    let formattedBook = book.replace(/[^a-zA-Z0-9 .\n]/g, '');
    let lines = formattedBook.split('\n').filter(line => line.trim() !== '');
    let currentBalance = parseFloat(lines[0]);
    let totalExpense = 0;
    let result = [`Original Balance: ${currentBalance.toFixed(2)}`];
    for (let i = 1; i < lines.length; i++) {
        ...
        totalExpense += amount;
        result.push(`${checkNumber} ${category} ${amount.toFixed(2)} Balance ${currentBalance.toFixed(2)}`)
    }
    let averageExpense = totalExpense / (lines.length - 1);
