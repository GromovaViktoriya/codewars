...
// races will certainly be involved. Each race has a certain worth when battling against others. On the side of good we
...
function goodVsEvil(good, evil) {
    ...
    const goodPoints = good.split(' ').reduce((acc, count, index) => {
        return acc + (count * goodWorth[index]);
    }, 0);
    const evilPoints = evil.split(' ').reduce((acc, count, index) => {
        return acc + (count * evilWorth[index]);
    }, 0);

    return goodPoints > evilPoints
        ? "Battle Result: Good triumphs over Evil"
        : goodPoints < evilPoints
