...
//
// If the sum of a chunk's digits is divisible by 2, reverse that chunk; otherwise rotate it to the left by one
...
function revrot(str, sz) {
    ...
    arr.forEach(chunk => {
        ...
        if (chunkArr.reduce((curr, item) => curr + Number(item), 0) % 2 === 0){
            resultStr += chunkArr.reverse().join('')
        } else {
            resultStr += (chunk.slice(1) + chunk.slice(0,1))
        }
    })
    return resultStr
}
