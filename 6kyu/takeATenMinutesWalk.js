...
function isValidWalk(walk) {
    if (walk.length === 10) {
        let southArr = walk.filter(d => d === 's' ? d : '')
        let northArr = walk.filter(d => d === 'n' ? d : '')
        let eastArr = walk.filter(d => d === 'e' ? d : '')
        let westArr = walk.filter(d => d === 'w' ? d : '')
        return eastArr.length === westArr.length && southArr.length === northArr.length
    }
    return false
}
