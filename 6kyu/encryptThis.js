...
let encryptThis = function (text) {
    return text.split(" ").map(word => word.length === 1
        ? word.charCodeAt(0)
        : word.length === 2
            ? word[0].charCodeAt(0) + word.slice(word.length-1)
            : word[0].charCodeAt(0) + word.slice(word.length-1) + word.slice(2, word.length-1) + word[1]).join(" ")
}
