...
let humanYearsCatYearsDogYears = function (humanYears) {
    let catAgeAfterTwo = 4
    let dogAgeAfterTwo = 5
    if (humanYears > 0 && humanYears === 1) return [1, 15, 15];
    else if (humanYears === 2) return [2, 24, 24];
    else return [humanYears, 24+(catAgeAfterTwo*(humanYears-2)), 24+(dogAgeAfterTwo*(humanYears-2))];
}
